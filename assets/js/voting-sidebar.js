(function () {
  ...
  function loadVotes() {
    ...
    // Read section counters directly to avoid downloading large voters maps.
    sections.forEach(function (s, idx) {
      var sectionRef = db.ref('votes/' + postSlug + '/' + s.id);

      sectionRef.child('up').on('value', function (snap) {
        s.up = Number(snap.val()) || 0;
        updatePctDisplay(idx);
        updateMobilePct(activeIdx);
      }, function (err) {
        console.warn('Voting sidebar: failed to read up-votes from Firebase:', err);
      });

      sectionRef.child('down').on('value', function (snap) {
        s.down = Number(snap.val()) || 0;
        updatePctDisplay(idx);
        updateMobilePct(activeIdx);
      }, function (err) {
