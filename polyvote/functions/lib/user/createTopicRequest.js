 ...
 */
exports.createTopicRequest = (0, https_1.onCall)(async (request) => {
    const uid = (0, adminOnly_1.requireAuth)(request);
    const db = (0, firestore_1.getFirestore)();
    const { title, description, category, metrics } = request.data;
    // Validate title
    ...
    if (trimTitle.length === 0 || trimTitle.length > 200) {
        throw new https_1.HttpsError("invalid-argument", "Title must be between 1 and 200 characters.");
    }
    ...
    if (description.trim().length === 0) {
        throw new https_1.HttpsError("invalid-argument", "Description cannot be empty.");
    }
    // Validate category
    ...
    if (!Array.isArray(metrics) || metrics.length === 0 || metrics.length > 6) {
        throw new https_1.HttpsError("invalid-argument", "Must have between 1 and 6 metrics.");
    }
    for (const m of metrics) {
        if (!m.id || !m.label || typeof m.label !== "string" || !m.label.trim()) {
            throw new https_1.HttpsError("invalid-argument", "Each metric must have a label.");
        }
        if (!Array.isArray(m.choices) ||
            m.choices.length < 2 ||
            m.choices.length > 6) {
            throw new https_1.HttpsError("invalid-argument", "Each metric must have 2-6 choices.");
        }
        for (const c of m.choices) {
            if (!c.id || !c.label || typeof c.label !== "string" || !c.label.trim()) {
                throw new https_1.HttpsError("invalid-argument", "Each choice must have a label.");
            }
            if (!c.color || typeof c.color !== "string") {
                throw new https_1.HttpsError("invalid-argument", "Each choice must have a color.");
            }
        }
    }
...
//# sourceMappingURL=createTopicRequest.js.map