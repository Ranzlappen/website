...
exports.onTopicRequestEndorsed = void 0;
 ...
 */
exports.onTopicRequestEndorsed = (0, firestore_1.onDocumentUpdated)("topicRequests/{requestId}", async (event) => {
    const before = event.data?.before.data();
    const after = event.data?.after.data();
    if (!before || !after)
        return;
    // Only act if endorsement count increased and threshold is now met
    if (after.endorsementCount <= before.endorsementCount)
        return;
    if (after.status !== "pending")
        return;
    if (after.endorsementCount < REQUEST_ENDORSEMENTS_NEEDED)
        return;
...
//# sourceMappingURL=onEndorse.js.map