...
- **Tabletop** (`games/`) — React 19 SPA built with Vite: a **reusable browser game engine for card and board games**. The engine core (`games/src/engine/`) is framework-agnostic TypeScript (state, players, turns, phases, a deterministic seeded RNG, pure `applyAction` reducer, serialization, undo/redo, registries, a **declarative ruleset layer** — `defineGame` compiles a map of moves with phase gates + `nextPhase`/`endsTurn` declarations into a plain `GameDefinition` — and a **zone subsystem** — named card zones with `public`/`owner`/`hidden` visibility, move/draw/auto-reshuffle ops, and automatic per-viewer redaction derived from the policies) with no React/Firebase imports; the React layer (`games/src/ui/`, incl. the modular `ui/assets/` art + table-primitive library), persistence (`games/src/storage/`) and a backend-agnostic multiplayer `SyncAdapter` (`games/src/net/` — a zero-config cross-tab Local adapter plus an optional Firebase RTDB adapter, default local) sit on top. Ships three playable demo games (Crown Rush — card; Lantern Hunt — board; Lantern/Relic Run — hybrid, all authored declaratively), three themes, an all-SVG/CSS asset library, and 100 Vitest tests. Modes: solo vs bots, hot-seat, online rooms (join by code/link, presence, ready, reconnect). Online has two trust models behind the one adapter interface (selected by `adapter.serverAuthoritative`): the default **Local** adapter is *client* host-authoritative (cross-tab), while the optional **Firebase** adapter is *server* authoritative — the engine is mirrored into `polyvote/functions/src/games/` and the `gamesCreateMatch`/`gamesSubmitAction` Cloud Functions validate + apply every move (actor = authenticated anon-auth uid) and write per-player redacted RTDB slots (clients read only their own; `database.rules.json` denies client state writes). Deployed as a subfolder within the Jekyll `_site/` at `/games/`. **See [`games/README.md`](./games/README.md) and the handbook at [`games/docs/wiki/README.md`](./games/docs/wiki/README.md)** (architecture, engine concepts, how to add a card/board/hybrid game, assets, themes, Firebase, testing, accessibility, performance, API reference).
...
- **Post status**: Posts use a `status` field in front matter (`published`, `draft`, `placeholder`, `unpublished`). Only `published` posts appear in the sitemap and feed; `placeholder` posts render in on-site listings (homepage, `/blog/`) but carry `noindex,nofollow` and stay out of sitemap/feed like drafts (see `_includes/head.html`, `sitemap.xml`, `feed.xml`).
...
- **Reference-table scaffolding**: The sticky tab strip + live-search + scrollable big-table pattern used by `/references/spectrum/`, `/references/cmd-cheat-sheet/`, and `/references/web-dev-tech-stack/` is shared. Styles live in `/assets/css/reference-table.css` (controls, tabs, search, the horizontal-only scroll wrapper + `.reference-thead-pin` clone-header scaffold, `.is-sticky-col` hook for the pinned column, generic legend swatches, `.reference-badge` base, the `.cell-collapse` family, print reset). Behaviour lives in `/assets/js/reference-table.js` and exposes one function — `window.initReferenceTable({tableId, tabSelector, searchId, countId, emptyId, blurbId, batchDataId, sortable, osFilterId, osAttr})` — that each page's thin wrapper calls with its own IDs. The last three are opt-in: `sortable: true` injects a click-to-sort control into every `<th>` (cycles ascending → descending → original; a cell's sort key is its `data-sort` attribute, else its text, numeric when both compared values parse as numbers; one column at a time), and `osFilterId`/`osAttr` wire a secondary `<select>` filter (the `.reference-osfilter` control) that ANDs a token match against a per-row attribute (default `data-os`) on top of the tab + search filters. The CLI cheat sheet passes all three; Spectrum passes none (unchanged). Per-page CSS files (`spectrum.css`, `cmd-cheat-sheet.css`, `web-dev-tech-stack.css`) layer column widths and palette modifiers on top, scoped under `.reference-page--<slug>` so overrides cannot leak between pages. The Web Dev Tech Stack page (`/references/web-dev-tech-stack/`, data in `_data/web-dev-tech-stack/*.yml`, ~230 technologies across 21 category tabs) is the newest example: it passes `sortable: true` plus reuses the generic `osFilterId`/`osAttr` secondary `<select>` as a **Layer** filter (`data-layer`), renders `maturity` / `learning_curve` / `layer` / `license` as colour-coded `.reference-badge` palettes, makes `prerequisites`/`pairs_well_with`/`alternatives` clickable cross-links to other rows' `#row-<slug>` anchors, and wires a per-row code `example` to `window.Glossary.openInline`. `_data/web-dev-tech-stack/lint.sh` enforces its quality bar (see that dir's README for the entry schema). To add a third reference page: opt in via `<section class="reference-page reference-page--<slug>">`, load the shared CSS/JS before the per-page ones, and write a thin wrapper that calls `initReferenceTable`. The shared CSS also carries a tablet breakpoint (`40.0625–64rem`, the old 641–1024px band, drops the search box onto its own full-width row; all blog breakpoints are rem tokens so they track user font scaling). **Sticky mechanism (reworked)**: the table wrapper is a **horizontal-only scroll box in normal page flow** (`overflow-x: auto; overflow-y: hidden`, no `position: sticky`, no `max-height`) — the page keeps one natural vertical scrollbar with no inner scrollbar or scroll trap. Because `overflow-x: auto` makes the wrapper its own scroll container, a real `thead` can never viewport-pin (CSS computes `overflow-y` to auto/hidden, never visible) — so `reference-table.js` builds a **pinned clone header**: `.reference-thead-pin`, a 0-height `position: sticky` rail inserted just before the wrapper (top = `--header-offset` + `--reference-controls-h`, z-index 9 under the z-10 controls), holding an `aria-hidden` clone of the thead inside an `overflow: hidden` viewport sized to the wrapper. The viewport's `scrollLeft` mirrors the wrapper's (passive scroll listener) — a real scrollport, so the cloned corner `th.is-sticky-col` keeps left-pinning via the same shared rule as the table's own sticky column. Column widths are copied from the real `th.offsetWidth`s (rAF-batched), resynced by ResizeObservers on table+wrapper **and** explicitly after every filter pass (hiding rows redistributes column widths without resizing the table's box). Visibility is IO-driven with the root spanning from the pin line down past the document, so "any part below the pin line" **is** the intersection state — instant jumps (anchor links, fast flicks, scroll restoration) can't skip the transition (a plain negative-top rootMargin misses them: below-viewport and above-pin-line are both "not intersecting"); the clone shows only while the real thead is above the pin line AND the wrapper still extends below it (never floats over legend/footer), and the observer is rebuilt on `headersticky:change` (header pin toggle) and whenever the controls strip resizes. Cloned sort buttons are `tabindex="-1"` and delegate clicks to the real header buttons; sort classes/`aria-sort` are mirrored back onto the clone. Search filtering is debounced (120 ms) over cached per-row lowercase text. A `@media print` block flattens all scroll/sticky chrome so the full tables print. `--reference-controls-h` is the **real measured height of the `.reference-controls` strip**, which `reference-table.js` publishes on `<html>` (once on init, then live via a `ResizeObserver` on the strip + a debounced `resize` listener) — so the clone pins *exactly* below the controls even when the mobile strip stacks to 2–3 rows; the `3.25rem` literal survives only as a CSS fallback (the pin itself exists only when JS runs). The controls strip's sticky `top` — and the abbreviations glossary's pinned summary — key off the global `--header-offset` custom property (defaults to `--header-height`), so unpinning the site header (which sets `--header-offset: 0` — see the header sticky-toggle under **Theme**) re-anchors everything to the very top. Mobile column snap is `proximity` (not `mandatory` — mandatory + the overlaying sticky first column made cell far-edges unreachable) with no `scroll-snap-stop`. `/assets/js/resize-handle.js` is a shared, self-attaching util that replaces the tiny native textarea resize corner with a large pointer+keyboard drag grabber on any `textarea[data-resize]` or `.cmd-widget__textarea`; load it after whatever builds the textareas. The CLI cheat sheet's table adds five columns beyond Spectrum's pattern — **OS / Appliance** (sortable; its `<select>` drives the secondary OS filter), **Danger** (sortable via a numeric `data-sort` rank: safe `0` < caution `1` < destructive `2`), **Recipes/Combos** (`recipes` = array of `{code, explain}`), **Modern alt** (`modern` string), and **Docs** (renders the existing `references` field) — and renders `examples` (now `{code, explain}` objects) as clickable buttons that open an explanation modal via `window.Glossary.openInline`. The per-row OS badge pills **and** the `danger` chip (`safe`/`caution`/`destructive`) *also* render stacked beneath the command name inside the sticky first column: this is intentional redundancy — the stacked copy stays visible during horizontal scroll, while the dedicated **OS / Appliance** and **Danger** columns are the sortable + filterable/searchable ones (each `<tr>` carries `data-os` + `data-danger` for the filter/sort, and the OS column renders text labels so search matches "linux", "macos", … too). **Raw-HTML cells**: `description`, flag `description`, `modern`, and `gotchas` are emitted as raw HTML so inline `<code>` works — literal placeholder angle brackets (`<script>`, `<path>`, …) must be escaped (`&lt;…&gt;`) in the data or they break the table DOM (a literal `<script>` swallows the rest of the table so the footer renders inside it). `_data/cmd-cheat-sheet/lint.sh` fails the build if any rawtext/script tag lands inside the command table.
...
- **Freelance project request tool (`/freelance/`)**: A pick-and-choose configurator (`pages/freelance.html`) for requesting freelance work — project type, grouped tech-stack chips, feature chips, AI-collaboration mode, timeline + price offer, and a live "your request so far" summary. Every chip/option is rendered by Liquid from `_data/freelance.yml`, so extending the picker is a data-only change (no HTML/JS edits). Styles in `assets/css/freelance.css` (scoped `.freelance-page`, theme-aware via the global CSS vars); behaviour in `assets/js/freelance.js`. Submission reuses the contact form's pipeline unchanged: hCaptcha behind the functional-consent gate, then POST to `site.contact_form.endpoint` when configured, else a pre-filled GitHub Issue URL (label `freelance-request` vs. the contact form's `contact-form`). The page opts out of the sticky-h2 banner via `page-body--no-sticky` (app-like layout; the full-bleed sticky h2 would break the summary sidebar). Linked from the main nav + footer ("Hire Me" in `_data/pages.yml`) and as the fourth hero button in `_includes/hero.html`.
...
| Styling | Custom CSS — main `style.css` (~3,200 lines), per-page stylesheets (`spectrum.css`, `electronics-fundamentals.css`, `cmd-cheat-sheet.css`), the shared `abbreviations.css`, and the shared `reference-table.css` (sticky-tab + live-search + pinned-clone-header big-table scaffolding used by Spectrum, the CLI cheat sheet, and the Web Dev Tech Stack) | Tailwind CSS v3 + Framer Motion | Tailwind CSS v4 (via `@tailwindcss/vite`) | Tailwind CSS v4 (via `@tailwindcss/vite`) | Tailwind CSS v4 + CSS-variable theme system (3 skins) |
...
| Editor | — | — | CodeMirror 6 | — | — |
| Testing | — | Vitest | — | — | Vitest (100 tests) |
...
│   ├── freelance.yml           # Freelance request tool option datasets (see Key Conventions)
│   ├── spectrum/               # EM spectrum band data + maintenance README
│   ├── web-dev-tech-stack/     # Web dev tech stack data (21 category files) + README + lint.sh
...
│   │   ├── freelance.css                # Freelance project request tool (/freelance/)
│   │   └── cookie-consent.css
│   ├── js/
...
│   │   ├── freelance.js                 # Freelance request tool (live summary, validation, submit)
...
│   ├── src/
│   │   ├── engine/             # Framework-agnostic core: types, rng, match (applyAction), client (undo/redo), serialize, registry, flow (defineGame), cards, zones, board, dice, rules
