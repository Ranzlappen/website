...
- **Brand splash / onboarding loader (global, blog)**: A full-screen animated loader of the brand emblem (the glowing blue glassy-shield "R" from the icon universe) shows **once per browser session** on the first blog page load. Assets live in `assets/images/loader/` — `brand-loader.mp4` (play-once H.264 video, ~36 KB; a 240×240, 30 fps clip **decimated to every 6th source frame so it plays through in ~1 s** — short on purpose so the once-per-session splash never delays the first paint) and `brand-loader-poster.png` (~23 KB static first-frame, used as the `<video poster>`, the `prefers-reduced-motion` fallback, and the instant placeholder). Shipped as a muted autoplay `<video playsinline>` — **no `loop`**, because the splash now plays through exactly once before dismissing (not an `<img>` because H.264 compresses this rendered 3D motion far smaller than an animated GIF/WebP would). The emblem is rendered large (`clamp(180px, 46vw, 300px)`) at `opacity: 0.88`. Gated like the theme/header bootstraps: `_includes/head.html` synchronously sets `<html data-splash="show">` before paint **only when `sessionStorage.brandSplashShown` is unset** (so it never flashes on repeat visits or no-JS loads). Markup + dismissal live in `_includes/brand-splash.html` (included at the top of `<body>` in `_layouts/default.html`, so all layouts inherit it); the inline script writes the `sessionStorage` flag immediately, then fades the overlay only once **both** gates clear — the page has fired `window.load` **and** the clip has played through once (the `video`'s `ended` event; reduced-motion or a missing/failed video satisfies this gate immediately and falls back to the `MIN_MS` 900 ms floor). A `MAX_MS` (9 s) backstop guarantees the overlay can never get stuck if a gate never fires. CSS in `style.css` (search `.brand-splash`): fixed `z-index: 100000` over everything, fixed dark-blue backdrop (`#050d19`, matching the emblem art's built-in tile so there's no seam on either theme), opacity fade; under `prefers-reduced-motion` it swaps to the static poster and drops the fade. Both loader assets are precached by `sw.js` (bump `CACHE_VERSION` when they change). To retune length/size, re-encode from the original ~6 s/30 fps master with ffmpeg — keep every Nth frame and hold the framerate to shorten it (current is every 6th → ~1 s: `-vf "select='not(mod(n\,6))',setpts=N/30/TB,scale=240:240:flags=lanczos" -r 30 -an -c:v libx264 -crf 28`) — then re-derive the poster from its first frame (`pngquant` it back down); a tiny H.264 clip beats any GIF/WebP — don't ship a raw GIF.
