...
| `search-crawl.yml` | Manual only (`workflow_dispatch`) | Runs the `search-crawler` module (Node 22, no install) with the auto-provided `GITHUB_TOKEN`, then opens a PR with the refreshed `search-external.json` via `peter-evans/create-pull-request` (main is protected, so it never pushes there directly). Merging the PR triggers `jekyll-gh-pages.yml` and redeploys with the new index. `contents: write` + `pull-requests: write`; `search-crawl` concurrency group. **One-time setup:** enable Settings → Actions → "Allow GitHub Actions to create and approve pull requests". The bot PR is GITHUB_TOKEN-authored, so CI does not auto-run on it (a data-only change — just merge it). | Opens a PR bumping `search-external.json` (no deploy itself). |
