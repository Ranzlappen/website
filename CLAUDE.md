...
- **Header sticky-toggle (global, blog)**: A pin/unpin button in the header's `.nav-actions` (`#header-sticky-toggle`, mirrors the theme toggle's localStorage + pre-paint bootstrap + two-icon swap) lets the visitor unpin the normally-`fixed` site header so it scrolls away with the page. State lives in `localStorage.headerSticky` (`on` default / `off`), bootstrapped synchronously in `_includes/head.html` (sets `<html data-header-sticky="off">` before paint) and toggled in `assets/js/main.js`. CSS in `style.css`: `html[data-header-sticky="off"]` sets `--header-offset: 0rem` (the default is `--header-offset: var(--header-height)`) and switches `.site-header` to `position: absolute` (so it scrolls away — `main#main-content`'s `padding-top` still reserves its height). The toggle stays **inline** with the search + theme buttons while the header is visible; only once the page is scrolled and the header has actually left the viewport (`html[data-header-sticky="off"].is-scrolled`, a class `main.js` toggles on every page from `scrollY`) does `.header-sticky-toggle` detach into a `position: fixed` corner floater (above content via z-index, half-opacity, full on hover/focus) so it stays reachable without scrolling back up. (Earlier this floater applied across the whole `off` state, which left the toggle detached and mis-aligned from the other header buttons even at the top of the page — gating it on `.is-scrolled` keeps it inline while the header is on screen.) `--header-offset` (not the raw `--header-height`) drives the global `scroll-padding-top` and the post-body `h2` `scroll-margin-top`, plus the reference pages' `.cmd-widget` / `.electronics-section` `scroll-margin-top` and the reference-table controls + wrapper (see **Reference-table scaffolding**), so unpinning re-anchors headings to the very top (no empty header-height gap above them) and reclaims that vertical space for the big tables.
